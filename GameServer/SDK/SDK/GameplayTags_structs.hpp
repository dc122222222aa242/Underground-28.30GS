...
public:
	...
	{
		for (auto i = 0; i < GameplayTags.Num(); i++) {
			if (GameplayTags[i] == TagToCheck)
				return true;
		}
			

		for (auto i = 0; i < ParentTags.Num(); i++) {
			if (ParentTags[i] == TagToCheck)
				return true;
		}
		return false;
	}
public:
	...
	{
		for (auto i = 0; i < ContainerToCheck.GameplayTags.Num(); i++) {
			auto& Tag = ContainerToCheck.GameplayTags[i];
			bool Found = false;

			for (auto i=0; i< GameplayTags.Num(); i++)
			{
				auto& Tag2 = GameplayTags[i];
				if (Tag2 == Tag)
				{
					Found = true;
					break;
	...
	{
		for (auto i=0; i< Other.GameplayTags.Num(); i++)
		{
			const auto& GameplayTag = Other.GameplayTags[i];
			if (!HasTag(GameplayTag))
				GameplayTags.Add(GameplayTag);
		}

		for (auto i=0; i< Other.ParentTags.Num(); i++)
		{
			const auto& ParentTag = Other.ParentTags[i];
			if (!HasTag(ParentTag))
