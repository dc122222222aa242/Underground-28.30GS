...
namespace UC
{
	...
	{
	public:
		static inline auto ReallocOG = (void* (*)(void*, __int64, unsigned int)) (__int64(GetModuleHandleW(0)) + 0x43DC21C);

		static __forceinline void* InternalRealloc(void* _a1, __int64 _a2, unsigned int _a3) {
			return ReallocOG(_a1, _a2, _a3);
		}
	...
	public:
		...
		inline TArray(int32 Size)
			: NumElements(0), MaxElements(Size), Data(reinterpret_cast<ArrayElementType*>(FMemory::Malloc(sizeof(ArrayElementType)* Size, alignof(ArrayElementType))))
		{
		}
	...
	public:
		void Append(TArray<ArrayElementType> OtherArray)
		{
			for (ArrayElementType ArrayElement : OtherArray)
			{
		...
		{
			if (Num + NumElements > MaxElements) Data = (ArrayElementType*)FMemory::Realloc(Data, (MaxElements = Num + NumElements) * sizeof(ArrayElementType), alignof(ArrayElementType));
		}
		...
		inline ArrayElementType& AddByIndex(const ArrayElementType& Element, int32 Index)
		{
			Reserve(1);
	...
	public:
		...
		ArrayElementType* Search(std::function<bool(ArrayElementType& val)> check) {
			for (auto& v : *this) {
				if (check(v)) return &v;
			}
			return nullptr;
		...
		int32_t SearchIndex(std::function<bool(ArrayElementType& val)> check) {
